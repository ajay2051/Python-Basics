...
def capitals(word):
    """ This function returns the capital letter of givem string """
    result = []
    for i in word:
        if i.isupper():
            result.append(i)
...
for word in sentence.split():
    frequency[word] = frequency.get(word, 0) + 1 # 0 is default value if word is not found if word is found then add + 1
print(frequency)
